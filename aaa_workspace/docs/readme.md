...
2. 子进程加载 `MjModel`、`MjData` 并运行物理循环；
3. 主进程通过 `MujocoServerProxies` 写入命令、读取状态；
4. 服务端控制器把命令转换成 MuJoCo actuator 控制量；
5. 相机和传感器 manager 更新数据快照；
6. 停止时设置 stop event，并等待子进程结束。
...
2. `office_*_semantics.json`：对象类型、body/geom 绑定、属性和交互点；
3. `office_*_agents.json`：员工画像、日程、需求、偏好和配置。
...
2. `mjmodel/mjdata` 的物理步进和渲染有线程/锁约束，违反时可能导致 MuJoCo 崩溃或数值不稳定。
3. 相机越多越慢，rangefinder 激光也较耗算力，应按需启用。
4. 语义 JSON 的 body/geom 名称必须和 XML 一致，否则 Agent 可能找到语义对象却找不到物理实体。
5. NPC 已有动作、导航和员工 Agent，但动作过渡、复杂社交、多 NPC 对话、失败恢复和长时间稳定性仍是建设重点。
6. 视觉 mesh、碰撞 proxy、交互 site 和实际摆放不一致时，导航、抓取和 handover 可能“看起来正确但执行失败”。
7. LLM 凭据应放在被忽略的 `office_llm.local.json`；SMPL-X 参数、checkpoint、数据集、录屏和生成输出不要提交。示例配置只作为格式模板使用。

## 十四、推荐上手顺序

1. 先运行 `examples/move_joints.py` 或 `examples/office_scene.py --headless`；
2. 阅读 `stretch_mujoco_simulator.py`，理解命令、状态和生命周期；
3. 阅读 `mujoco_server.py`，理解进程、代理和物理循环；
4. 阅读 `robots/base.py` 和 `robots/stretch3/`；
5. 用 `tests/test_robot_interfaces.py`、`test_office_scene.py`、`test_office_semantics.py` 熟悉行为入口；
6. 再按任务进入 `navigations/`、`graspgen/`、`humanoid/`、`agents/` 或 `semantics/`；
7. 最后阅读 `strech_codex/`，理解 MCP 工具编排。
...
- [NPC 系统优化说明](../task/npc_system_optimization.md)：当前 NPC 待办和优化方向；
