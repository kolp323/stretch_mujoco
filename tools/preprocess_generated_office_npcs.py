...
def _location_slots(
    ...
    *,
    count: int = 3,
    separation: float = 0.65,
) -> tuple[tuple[float, float], ...]:
    """Derive reproducible, separated occupancy slots around a route anchor."""
    slots = [anchor]
    while len(slots) < count:
        slots.append(_choose(candidates, slots, separation=separation))
    return tuple(slots)


def _facing_yaw(origin: tuple[float, float], target: tuple[float, float]) -> float:
    """Return the NPC root yaw whose local forward (-Y) points at ``target``."""
...
def _oriented_site(
    ...
    """Project a navigation point to a site pose facing its owned furniture."""
    return (*point, _facing_yaw(point, target))
...
def _append_sites(
    ...
    # The native NPC renderer requests this stable camera identifier.  Generated
    # source scenes already provide an authored ``overview`` camera, so clone
    # it in the derivative instead of inventing a scene-specific camera pose.
    cameras = {node.get("name"): node for node in worldbody.findall("camera")}
    if "office_overview" not in cameras:
        overview = cameras.get("overview")
        if overview is None:
            raise ValueError(f"Generated scene has no overview camera: {source}")
        office_overview = copy.deepcopy(overview)
        office_overview.set("name", "office_overview")
        worldbody.append(office_overview)
...
def _semantic_world(
    ...
    for zone, target, role, owner in (
        ("meeting", "meeting_table", "meeting_place_site", "meeting_table"),
        ("lounge", "lounge", "human_stand_site", "lounge"),
        ("snack", "snack_counter", "human_stand_site", "snack_counter"),
    ):
        for index in range(1, 4):
            site = f"npc_{zone}_site" if index == 1 else f"npc_{zone}_slot_{index:02d}_site"
            points[f"{zone}_slot_{index:02d}"] = {
                "role": role,
                "owner": owner,
                "site": site,
                "attributes": {
...
def _preprocess(scene: Path, output: Path) -> None:
    ...
    source_bodies = {
        "work": _body_for_asset(model, data, _asset_by_category(manifest, "workstation_pods")),
        "meeting": _body_for_asset(model, data, _asset_by_category(manifest, "meeting_tables")),
        "lounge": _body_for_asset(model, data, _asset_by_category(manifest, "legacy_sofas")),
        "snack": _body_for_asset(
            model, data, _asset_by_category(manifest, "furniture/snack_counter")
        ),
    }
    zone_targets = {
        zone: tuple(float(value) for value in data.xpos[model.body(body).id, :2])
        for zone, body in source_bodies.items()
    }
    candidates = {
    ...
    meeting_slots = _location_slots(candidates["meeting"], meeting)
    lounge_slots = _location_slots(candidates["lounge"], lounge)
    snack_slots = _location_slots(candidates["snack"], snack)
    ...
    sites = {
        "npc_work_site": _oriented_site(work, zone_targets["work"]),
        "npc_meeting_site": _oriented_site(meeting, zone_targets["meeting"]),
        "npc_lounge_site": _oriented_site(lounge, zone_targets["lounge"]),
        "npc_snack_site": _oriented_site(snack, zone_targets["snack"]),
        "npc_spawn_01_site": _oriented_site(spawn_1, zone_targets["work"]),
        "npc_spawn_02_site": _oriented_site(spawn_2, zone_targets["lounge"]),
        "npc_spawn_03_site": _oriented_site(spawn_3, zone_targets["snack"]),
        "npc_conversation_speaker_site": (*conversation[0], _facing_yaw(*conversation)),
        "npc_conversation_listener_site": (
            *conversation[1],
            _facing_yaw(conversation[1], conversation[0]),
        ),
        "npc_handover_giver_site": (*handover[0], _facing_yaw(*handover)),
        "npc_handover_receiver_site": (*handover[1], _facing_yaw(handover[1], handover[0])),
        "npc_stretch_request_site": _oriented_site(request, zone_targets["work"]),
        "npc_stretch_delivery_site": _oriented_site(delivery, zone_targets["work"]),
        "npc_stretch_rendezvous_site": _oriented_site(rendezvous, zone_targets["meeting"]),
    }
    for zone, slots in (
        ...
        for index, point in enumerate(slots[1:], 2):
            sites[f"npc_{zone}_slot_{index:02d}_site"] = _oriented_site(point, zone_targets[zone])
